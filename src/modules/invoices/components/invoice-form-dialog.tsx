...
import { createInvoiceAction, updateInvoiceAction } from '../invoices.actions';
import type { InvoiceWithItems } from '../invoices.service';
import { invoiceFormSchema, type InvoiceFormValues, type InvoiceInput } from '../invoices.validation';

interface Props {
  /** Omitted or null → create; an invoice → edit. Edit only ever reaches a draft — the table hides it otherwise. */
...
function TextField({
  ...
  name: FieldPath<InvoiceFormValues>;
  label: string;
  placeholder?: string;
  type?: string;
  disabled?: boolean;
...
export function InvoiceFormDialog({
  ...
  /** New invoice only: picking a client recomputes the due date from its payment terms (or the 30-day fallback). */
  function onClientChange(value: string) {
    form.setValue('contactId', '', { shouldValidate: false });

    if (isEdit) return;

    const client = clientOptions.find((option) => option.id === value);
    const termsDays = client?.paymentTermsDays ?? FALLBACK_PAYMENT_TERMS_DAYS;
    const issueDateValue = String(form.getValues('issueDate') ?? '');
    const issueDate = issueDateValue ? new Date(issueDateValue) : new Date();

    form.setValue('dueDate', toDateInputValue(addDays(issueDate, termsDays)), { shouldValidate: false });
  }
  ...
  function onSubmit(values: InvoiceInput) {
    ...
    startTransition(async () => {
      const result = isEdit
        ? await updateInvoiceAction({ invoiceId: invoice!.id, ...values })
    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit ? 'Update this draft invoice.' : 'Bill a client directly.'}
          </DialogDescription>
        </DialogHeader>
