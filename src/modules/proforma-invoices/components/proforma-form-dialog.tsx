...
import { createProformaAction, updateProformaAction } from '../proforma-invoices.actions';
import type { ProformaWithItems } from '../proforma-invoices.service';
import {
  PROFORMA_EDITABLE_STATUSES,
  proformaFormSchema,
...
interface Props {
  /** Omitted or null → create; a proforma → edit. One form for both. */
...
function TextField({
  ...
  name: FieldPath<ProformaFormValues>;
  label: string;
  placeholder?: string;
  type?: string;
  disabled?: boolean;
...
export function ProformaFormDialog({
  proforma,
  clientOptions,
  projectOptions,
  contactsByClient,
  ...
  const form = useForm<ProformaFormValues, unknown, ProformaInput>({
    ...
    defaultValues: {
      ...
      status: proforma && proforma.status !== 'converted' ? proforma.status : 'draft',
      issueDate: toDateInputValue(proforma?.issueDate) || toDateInputValue(new Date()),
      validUntil: toDateInputValue(proforma?.validUntil),
      currency: proforma?.currency ?? defaultCurrency,
  ...
  function onSubmit(values: ProformaInput) {
    setFormError(null);

    startTransition(async () => {
      const result = isEdit
        ? await updateProformaAction({ proformaInvoiceId: proforma!.id, ...values })
        : await createProformaAction(values);
      ...
      toast.success(isEdit ? 'Proforma invoice updated.' : 'Proforma invoice created.');
      onOpenChange(false);
      router.refresh();
    });
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>{isEdit ? `Edit ${proforma!.number}` : 'New proforma invoice'}</DialogTitle>
          <DialogDescription>
            {isEdit ? 'Update this proforma invoice.' : 'Draft a proforma invoice for a client.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4" noValidate>
            ...
            {proforma?.status === 'converted' && (
              <Alert>
                <AlertDescription>
                  This proforma has already been converted to an invoice. Its status can no longer be changed
                  from here.
                </AlertDescription>
              </Alert>
            )}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      ...
                      value={field.value}
                      disabled={isPending || proforma?.status === 'converted'}
                    >
                      <FormControl>
                        <SelectTrigger className="w-full">
                          <SelectValue />
                      ...
                      <SelectContent>
                        {PROFORMA_EDITABLE_STATUSES.map((status) => (
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isPending || (isEdit && !form.formState.isDirty)}>
                ...
                {isPending ? 'Saving...' : isEdit ? 'Save' : 'Create proforma invoice'}
              </Button>
            </DialogFooter>
          </form>
        </Form>
